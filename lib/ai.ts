...
import { withRetry, classifyError, GenerationError, ErrorType } from "./errors"
...
export async function generateStoryText(userPrompt: string, level: string): Promise<GeneratedStory> {
  if (!apiKey) {
    throw new GenerationError("GOOGLE_API_KEY is not configured", ErrorType.API_KEY_MISSING, false)
  }
...
Sors UNIQUEMENT du JSON compact au format: { "title": string, "paragraphs": string[] } sans texte supplémentaire.`
  ...
  return withRetry(async () => {
    try {
      ...
      try {
        ...
        console.warn("Failed to parse JSON response, falling back to text splitting:", parseError)
      }
  ...
  }, 1, 2000) // Retry once with 2 second delay
}
...
French sentence: ${sentence}`
  ...
  return withRetry(async () => {
    try {
      ...
      try {
        ...
        console.warn("Failed to parse JSON response for definition, falling back to text splitting:", parseError)
      }
  ...
  }, 1, 1000) // Retry once with 1 second delay
}
